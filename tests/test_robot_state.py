...
from franka_sim.franka_protocol import RobotMode, MotionGeneratorMode, ControllerMode
...
def test_robot_state_mode_changes():
    ...
    print(f"\nTotal packed state size: {total_size} bytes")
    ...
    print(f"Offsets from end:")
    print(f"success_rate: -{8}")
    print(f"robot_mode: -{9}")
    print(f"reflex_reason: -{50}")
    print(f"errors: -{91}")
    print(f"controller_mode: -{93}")
    
    # Extract and verify each field
    motion_mode, controller_mode = struct.unpack('<BB', packed_state[controller_mode_offset:controller_mode_offset+2])
    errors = struct.unpack('<41B', packed_state[errors_offset:errors_offset+41])
    reflex_reason = struct.unpack('<41B', packed_state[reflex_reason_offset:reflex_reason_offset+41])
...
def test_robot_state_transformation_matrices():
    ...
    test_matrix = [
        1.0, 0.0, 0.0, 1.0,  # Last column is translation
        0.0, 1.0, 0.0, 2.0,
        0.0, 0.0, 1.0, 3.0,
        0.0, 0.0, 0.0, 1.0
    ]
