...
attached to a :class:`~franka_sim.mobile.common.SceneView`, so the arm
...
:class:`~franka_sim.mobile.duo_sim.MobileDuoScene` (Genesis) or
:class:`~franka_sim.mobile.duo_mujoco_sim.MobileDuoMujocoScene` (MuJoCo) -- so
this module imports only :mod:`franka_sim.mobile.common` (genesis-free) at
...
from franka_sim.mobile.common import ROLE_BASE, ROLES
...
def parse_bind_specs(values: Sequence[str]) -> Dict[str, str]:
    ...
    Every role in :data:`~franka_sim.mobile.common.ROLES` must be present
    exactly once.
    """
    binds: Dict[str, str] = {}
    for value in values:
