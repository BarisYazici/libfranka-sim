...
#
# The controller solves every commanded ``O_T_EE_c`` to joint space and holds
# the per-cycle change of the IK joint velocity (rad/s^2 -> error 29) and of
# the IK joint acceleration (rad/s^3 -> error 30) to per-joint limits
# (:meth:`~franka_sim.limits.checker.MotionLimitChecker
# ._check_cartesian_joint_continuity`). Which robot's limits depends on the
# FCI version the server speaks: :func:`joint_discontinuity_limits`.
#
# Calibration: FER (Panda, FCI v5), left arm, 2026-09-08 -- a +x ``limitRate``
...
#: The FER's ``franka::kMaxJointJerk`` (libfranka 0.9), rad/s^3.
LIBFRANKA_FER_JOINT_JERK_LIMITS = (7500.0, 3750.0, 5000.0, 6250.0, 7500.0, 10000.0, 10000.0)

#: Calibrated multiplier on the tables above: 1.0 on the FER (measured), 1.0
#: on the FR3 (published, unconfirmed). Run-time re-tuning is
#: :data:`JOINT_DISCONTINUITY_SCALE_ENV_VAR`.
CARTESIAN_JOINT_DISCONTINUITY_FACTOR = 1.0


def _scaled(limits: Sequence[float]) -> Tuple[float, ...]:
    return tuple(CARTESIAN_JOINT_DISCONTINUITY_FACTOR * limit for limit in limits)


#: Per-joint bound on the IK joint acceleration (error 29) and jerk (30) of a
#: Cartesian stream, per robot. The FR3's are libfranka >= 0.10's uniform
#: :data:`MAX_JOINT_ACCELERATION` / :data:`MAX_JOINT_JERK`.
FER_JOINT_VELOCITY_DISCONTINUITY_LIMIT = _scaled(LIBFRANKA_FER_JOINT_ACCELERATION_LIMITS)
FER_JOINT_ACCELERATION_DISCONTINUITY_LIMIT = _scaled(LIBFRANKA_FER_JOINT_JERK_LIMITS)
FR3_JOINT_VELOCITY_DISCONTINUITY_LIMIT = _scaled(MAX_JOINT_ACCELERATION)
FR3_JOINT_ACCELERATION_DISCONTINUITY_LIMIT = _scaled(MAX_JOINT_JERK)

#: The two tables by FCI protocol version: 5 is the FER (Panda), 10 the FR3.
JOINT_DISCONTINUITY_LIMITS_BY_PROTOCOL = {
    5: (FER_JOINT_VELOCITY_DISCONTINUITY_LIMIT, FER_JOINT_ACCELERATION_DISCONTINUITY_LIMIT),
    10: (FR3_JOINT_VELOCITY_DISCONTINUITY_LIMIT, FR3_JOINT_ACCELERATION_DISCONTINUITY_LIMIT),
}


def joint_discontinuity_limits(
    protocol_version: int,
) -> Tuple[Tuple[float, ...], Tuple[float, ...]]:
    """The joint-side tables for the robot behind an FCI ``protocol_version``."""
    try:
        return JOINT_DISCONTINUITY_LIMITS_BY_PROTOCOL[protocol_version]
    except KeyError:
        raise ValueError(
            f"unsupported protocol version {protocol_version!r}; "
            f"supported: {tuple(JOINT_DISCONTINUITY_LIMITS_BY_PROTOCOL)}"
        ) from None


#: Environment variable holding a run-time multiplier on both joint-side
#: thresholds (default 1.0): ``FRANKA_SIM_JOINT_DISCONTINUITY_SCALE=0.8``
#: tightens them to 80 %. ``--joint-discontinuity-scale`` wins over it.
...
#: :func:`joint_discontinuity_limits` for the calibration.
