...
from franka_sim.franka_protocol import (
    ...
    RbkControllerMode, RbkMotionGeneratorMode, convert_to_rbk_motion_mode, convert_to_rbk_controller_mode,
    SetCollisionBehaviorCommand
)
from franka_sim.robot_state import RobotState
from franka_sim.franka_genesis_sim import FrankaGenesisSim, ControlMode
...
class FrankaSimServer:
    ...
    def _handle_commands(self):
        ...
        try:
            print("Starting UDP command polling...")
            # Setup poll object for UDP socket
            poller = select.poll()
            print(f"Command socket file descriptor: {self.udp_socket.fileno()}")
            ...
            while self.running:
                ...
                for fd, event in events:
                    ...
                    try:
                        # print("Waiting for UDP command polling...")
                        data, addr = self.udp_socket.recvfrom(expected_size)
                        # print(f"Received data: {data}")
                        if len(data) != expected_size:
                            logger.warning(f"Got a UDP packet with wrong size! Expected {expected_size}, got {len(data)}")
                            continue
                ...
                if command and command['message_id'] > 0:
                    ...
                    if command['motion_generation_finished']:
                        ...
                        if self.control_mode != ControlMode.POSITION:
                            logger.info("Motion finished: Switching to position control mode and holding current position")
                    ...
                    if self.robot_state.state['controller_mode'] == RbkControllerMode.kJointImpedance and \
                       self.robot_state.state['motion_generator_mode'] == RbkMotionGeneratorMode.kJointPosition:
                        if self.control_mode is not ControlMode.POSITION:
                            print("Setting control mode to POSITION")
                            self.genesis_sim.set_control_mode(ControlMode.POSITION)
                            self.control_mode = ControlMode.POSITION
                        ...
                        self.genesis_sim.update_torques([0.0] * 7)
                    elif self.robot_state.state['controller_mode'] == RbkControllerMode.kJointImpedance and \
                         self.robot_state.state['motion_generator_mode'] == RbkMotionGeneratorMode.kJointVelocity:
                        if self.control_mode is not ControlMode.VELOCITY:
                            print("Setting control mode to VELOCITY")
                        ...
                        self.genesis_sim.update_torques([0.0] * 7)
                    elif self.robot_state.state['controller_mode'] == RbkControllerMode.kExternalController:
                        if self.control_mode is not ControlMode.TORQUE:
                            print("Setting control mode to TORQUE")
    ...
    def handle_move_command(self, client_socket, header: MessageHeader, payload: bytes) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error handling Move command: {move_cmd.controller_mode} is not a valid ControllerMode")
                self.send_move_response(
                    client_socket,
                    command_id=header.command_id,
                    status=MoveStatus.kInvalidArgumentRejected
            ...
            # Update robot state
            self.robot_state.set_motion_generator_mode(convert_to_rbk_motion_mode(move_cmd.motion_generator_mode))
            self.robot_state.set_controller_mode(convert_to_rbk_controller_mode(move_cmd.controller_mode))
            ...
            if move_cmd.controller_mode == ControllerMode.kJointImpedance and move_cmd.motion_generator_mode == MotionGeneratorMode.kJointPosition:
                print("Setting control mode to POSITION")
                self.genesis_sim.set_control_mode(ControlMode.POSITION)
                self.control_mode = ControlMode.POSITION
            elif move_cmd.controller_mode == ControllerMode.kJointImpedance and move_cmd.motion_generator_mode == MotionGeneratorMode.kJointVelocity:
                print("Setting control mode to VELOCITY")
                self.genesis_sim.set_control_mode(ControlMode.VELOCITY)
                self.control_mode = ControlMode.VELOCITY
    ...
    def handle_set_collision_behavior_command(self, client_socket, header: MessageHeader, payload: bytes):
        ...
        try:
            ...
            total_size = 12 + 4  # Header (12) + status (1) + padding (3)
            response_header = MessageHeader(Command.kSetCollisionBehavior, header.command_id, total_size)
            header_bytes = response_header.to_bytes()
            response_data = struct.pack('<B3x', 0)  # 1 byte status + 3 bytes padding
        ...
        try:
            ...
            while self.running and self.transmitting_state:
                ...
                if time.time() - last_stats_time >= 1.0:
                    avg_genesis_time = (total_genesis_time / total_cycles) * 1000  # Convert to ms
                    avg_cycle_time = (total_cycle_time / total_cycles) * 1000  # Convert to ms
                    freq = total_cycles / (time.time() - last_stats_time)
                    
                    logger.info(f"State Update Stats - Freq: {freq:.1f}Hz, "
        ...
        try:
            ...
            try:
                ...
                if e.errno == 48:  # Address already in use
                    logger.warning(f"Port {self.port} is in use, attempting to force close and rebind...")
    ...
    def start(self):
        ...
        try:
            self.running = True
            print("Starting server and simulation...")

            # Initialize Genesis simulator first
            self.genesis_sim.initialize_simulation()
            print("Genesis simulation initialized")
            ...
            if self.genesis_sim.enable_vis:
                ...
                print("Starting Genesis simulator with visualization")
                self.genesis_sim.start()
            else:
                # Without visualization, run server in main thread
                print("Starting TCP/UDP server")
                self.run_server()
