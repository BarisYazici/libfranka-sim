...
``from franka_sim.mobile_duo_sim import X`` keeps working.
"""
...
class SceneView:
    ...
    on ``scene`` (either :class:`~franka_sim.mobile_duo_sim.MobileDuoScene` or
    :class:`~franka_sim.mobile_duo_mujoco_sim.MobileDuoMujocoScene`), so it
