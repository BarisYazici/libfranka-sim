...
# the viewer (as on the real hand, a grasp closes the fingers until they stall
# and succeeds only if the stall width is within epsilon of the commanded one,
# so grasping thin air returns false)
...
- [x] The joint side of the Cartesian generators: every commanded pose is solved to joint space and its joint velocity / acceleration steps judged per joint (`cartesian_motion_generator_joint_velocity_discontinuity` / `..._joint_acceleration_discontinuity`), against the simulated robot's own tables — the FER's when serving FCI v5 (calibrated against a real Panda), the FR3's published limits when serving v10 (**not yet confirmed against an FR3**) — `--joint-discontinuity-scale` / `FRANKA_SIM_JOINT_DISCONTINUITY_SCALE` to re-tune
